module.exports = "<div class='game-content progress-bar'>" +
   "<div class='instruction appear-card'>" +
      "<div class='press-txt'>" +
         "<p>Press alternatively the key</p>" +
         "</div>" +
      "<div class='touch-pad unique-pad'>" +
         "<div class='touch first-touch haut-touch'></div>" +
         "<div class='touch gauche-touch active-touch-left'></div>" +
         "<div class='touch bas-touch'></div>" +
         "<div class='touch droite-touch active-touch-right'></div>" +
         "</div>" +
      "<div class='bar-exemple'></div>"+
      "</div>"+
   "<div class='players-container'>" +
      "<div class='player-cont player-1 appear-card'>" +
         "<div class='container-bar'>" +
            "<p class='player-txt'>PLAYER 1</p>" +
            "<p class='validate'>VALIDATE</p>" +
            "<div class='validate-icon'><img src='' alt='validate-icon'></div>" +
            "<div class='bar-progress-cont-2'>" +
               "<div class='bar-progress-2'></div>" +
               "</div>" +
            "</div>" +
         "</div>" +
      "<div class='player-cont player-2 appear-card'>" +
         "<div class='container-bar'>" +
            "<p class='player-txt'>PLAYER 2</p>" +
            "<p class='validate'>VALIDATE</p>" +
            "<div class='validate-icon'><img src='' alt='validate-icon'></div>" +
            "<div class='bar-progress-cont-2'>" +
               "<div class='bar-progress-2'></div>" +
               "</div>" +
            "</div>" +
         "</div>" +
      "<div class='player-cont player-3 appear-card'>" +
         "<div class='container-bar'>" +
            "<p class='player-txt'>PLAYER 3</p>" +
            "<p class='validate'>VALIDATE</p>" +
            "<div class='validate-icon'><img src='' alt='validate-icon'></div>" +
            "<div class='bar-progress-cont-2'>" +
               "<div class='bar-progress-2'></div>" +
               "</div>" +
            "</div>" +
         "</div>" +
      "<div class='player-cont player-4 appear-card'>" +
         "<div class='container-bar'>" +
            "<p class='player-txt'>PLAYER 4</p>" +
            "<p class='validate'>VALIDATE</p>" +
            "<div class='validate-icon'><img src='' alt='validate-icon'></div>" +
            "<div class='bar-progress-cont-2'>" +
               "<div class='bar-progress-2'></div>" +
               "</div>" +
            "</div>" +
         "</div>" +
      "</div>"+
   "</div>";